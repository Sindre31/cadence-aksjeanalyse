...
export function Heatmap({ matrix, daysAxis, hoursAxis }) {
  const tip = useTip();
  return (
    <div className="heat" data-tipwrap>
      <div className="heat-grid" style={{ gridTemplateColumns: `36px repeat(${hoursAxis.length}, 1fr)` }}>
        <div className="heat-corner"></div>
        {hoursAxis.map(h => <div key={h} className="heat-colhead">{h}</div>)}
        {matrix.map((row, di) => (
          <React.Fragment key={di}>
            <div className="heat-rowhead">{daysAxis[di]}</div>
            {row.map((e, hi) => (
              <div
                key={hi}
                className="heat-cell"
                ...
                onMouseMove={(ev) => tip.show(ev, (
                  <span><b>{daysAxis[di]} {hoursAxis[hi]}:00</b><br />Edge score {e}/100</span>
                ))}
                onMouseLeave={tip.hide}
              >
                <span className="heat-val">{e}</span>
              </div>
            ))}
          </React.Fragment>
        ))}
